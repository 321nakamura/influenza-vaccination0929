// app.js（WebアプリURLを組込み済み）
'use strict';

const CONFIG = {
  storageKey: 'flu2025_submissions',
  webhookUrl: 'https://script.google.com/macros/s/AKfycbwg776wBEnqPRGZEtBUeiSRraZ5SQBrxoc3qYqFOj0YGOLYTnjW-MN0mZzlRiKoF_FDZQ/exec',
  remoteJsonUrl: 'https://script.google.com/macros/s/AKfycbwg776wBEnqPRGZEtBUeiSRraZ5SQBrxoc3qYqFOj0YGOLYTnjW-MN0mZzlRiKoF_FDZQ/exec'
};
...
function initDepartmentOptions(){ if(!els.department) return; els.department.innerHTML='<option value="">'+'選択してください</option>'+DEPARTMENTS.map(d=>`<option value="${d}">${d}</option>`).join(''); }
...
async function loadRemoteAll(){
  try{ const r=await fetch(CONFIG.remoteJsonUrl,{cache:'no-store'}); const j=await r.json(); if(j && j.ok && Array.isArray(j.items)) return j.items; }catch(e){ console.warn('remote load error', e); }
  return [];
}
...
function addRow(item){
  ...
  tr.innerHTML=`<td>${esc(item.employeeId)}</td><td>${esc(item.department)}</td><td>${esc(item.fullName)}</td><td>${esc(item.choice)}</td><td>${esc(item.place||'')}</td><td>${item.createdAt?new Date(item.createdAt).toLocaleString('ja-JP'):''}</td>`;
...
document.addEventListener('DOMContentLoaded',()=>{
  els={form:q('reservation-form'),employeeId:q('employeeId'),department:q('department'),fullName:q('fullName'),choice:q('choice'),place:q('place'),listTbody:document.querySelector('#list tbody'),exportCsvBtn:q('exportCsvBtn'),clearBtn:q('clearBtn'),csvFile:q('csvFile'),adminToggle:q('adminToggle')};
